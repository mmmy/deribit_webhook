...
export class DeribitClient {
  ...
  async getOptionDetails(instrumentName: string): Promise<OptionDetails | null> {
    try {
      ...
      console.error(`Failed to get option details for ${instrumentName}:`, error);
      return null;
    }
  }
  ...
  async getInstrumentByDelta(currency: string, minExpiredDays: number, delta: number, longSide: boolean): Promise<DeribitOptionInstrument | null> {
    try {
      console.log(`🔍 Finding option by delta: ${currency}, minExpiredDays: ${minExpiredDays}, delta: ${delta}, longSide: ${longSide}`);
      ...
      console.log(`📊 Filtered by option type (${optionType}): ${filteredInstruments.length} instruments`);
      ...
      for (const expiryTimestamp of nearestTwoExpiries) {
        ...
        console.log(`📊 Processing ${instrumentsForExpiry.length} instruments for expiry ${new Date(expiryTimestamp).toLocaleDateString()}`);
        ...
        for (const instrument of instrumentsForExpiry) {
          try {
            ...
            console.warn(`⚠️ Failed to get details for ${instrument.instrument_name}:`, error);
          }
        }
        ...
        console.log(`🎯 Selected ${top2ForExpiry.length} options for expiry ${new Date(expiryTimestamp).toLocaleDateString()}`);
        top2ForExpiry.forEach(option => {
          console.log(`   - ${option.instrument.instrument_name} (Delta: ${option.details.greeks.delta.toFixed(3)}, Distance: ${option.deltaDistance.toFixed(3)})`);
        });
      ...
      console.log(`✅ Selected option: ${bestOption.instrument.instrument_name}`);
      console.log(`📊 Delta: ${bestOption.details.greeks.delta} (target: ${delta}, distance: ${bestOption.deltaDistance})`);
      console.log(`💰 Spread ratio: ${(bestOption.spreadRatio * 100).toFixed(2)}%`);
